...
class TeleHealthcareFlowEditTECUser extends PolymerElement {
  static get template() {
    return html`
  ...
  _handleResponse(e) {
    ...
    this.dispatchEvent(new CustomEvent("edited-user", { detail: { 'message': response }}));
  }

  edittecuser(sp, email, name, phone, role) {
      this.$.client._dataChanged();
      this._postEvent = "EditTecUser";
      var postData = {};
      ...
      var postTo = {};
      postTo['Profile'] = email;

      this.$.client.postSmart(postTo, postData);
  }
}

window.customElements.define('telehealthcareflow-edittecuser', TeleHealthcareFlowEditTECUser);
